...
public class MonHocRepositoryImpl implements MonHocRepository {
    ...
    public List<MonhocHocky> getMonHocByGiangVien(Map<String, String> params) {
        ...
        if (params != null) {
            ...
            q.select(rMonHoc).where(predicates.toArray(new Predicate[0]));
    ...
    public List<MonhocHocky> getMonHocHocKy(Map<String, String> params) {
        ...
        Sinhvien sinhvien = sinhVienRepository.getSinhVien(idTaiKhoan);
        String idLopHoc = sinhvien.getMaLop().getIdLopHoc().toString();
        Root rMonHocHocKy = q.from(MonhocHocky.class);
        List<Predicate> predicates = new ArrayList<>();
        if (idLopHoc != null) {
            predicates.add(b.equal(rMonHocHocKy.get("idHocky").get("idLop").get("idLopHoc"), Integer.parseInt(idLopHoc)));
        }
        if (tenMonHoc != null) {
        ...
        q.select(rMonHocHocKy).where(predicates.toArray(new Predicate[0]));
        Query query = s.createQuery(q);
        try {
            List<MonhocHocky> monhocHockys = query.getResultList();
            return monhocHockys;
    ...
    public List<Monhocdangky> getMonHocSinhVienDangKy(Map<String, String> params) {
        ...
        List<Predicate> predicates = new ArrayList<>();
        String idSv = taiKhoanRepository.getTaiKhoan(idSinhVien).getSinhvien().getIdSinhVien();
        if (idSv != null) {
            predicates.add(b.equal(rMonHocHocKy.get("idMonHocHocKy"), rMonHocDangKy.get("idMonHoc")));
            predicates.add(b.equal(rMonHocDangKy.get("idSinhVien"), idSv));
        }
        ...
        q.select(rMonHocDangKy).where(predicates.toArray(new Predicate[0]));
        q.distinct(true);
        Query query = s.createQuery(q);
        try {
            return query.getResultList();
